...
- **自定义播放 UA**：可在“设置”中填写播放请求 User-Agent，用于需要指定 UA 的直播源。
...
7. **自定义播放 UA**
   - 点击顶部“⚙️”打开“设置”，在“自定义播放 UA”中填写 User-Agent 并保存；
   - 受浏览器限制，网页无法可靠直接修改 `User-Agent`。如需完全生效，请启动或部署播放代理，并在“播放代理模板”中填写类似：`http://localhost:8789/proxy?url={url}&ua={ua}`；
   - 保存后会重新加载当前频道，后续播放 URL 会通过代理转发，代理会用填写的 UA 请求直播源。
...
- **播放 UA 代理**：如果直播源要求指定 UA，可使用仓库内置的零依赖代理示例：
  ```bash
  node server/playback-proxy.js
  ```
  然后在设置中填写代理模板：`http://localhost:8789/proxy?url={url}&ua={ua}`。生产部署时建议把代理部署到自己的服务器并限制可访问的上游域名。
